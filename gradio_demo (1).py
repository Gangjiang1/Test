...
from transformers import (
    ...
    # LlamaTokenizer,
    StoppingCriteria,
    # 用于指定在生成任务中停止生成的条件。例如，可以设置生成的文本长度或其他条件，以便在满足条件时停止生成。
)
import gradio as gr
import argparse
import os
...
def setup():
    ...
    # load_type = torch.float32  # 设置为 float32，即全精度浮点数
    ...
    # 我们也应该把tokenizer下载到本地
    base_model = AutoModelForSeq2SeqLM.from_pretrained(
        args.base_model,
        # load_in_8bit=load_in_8bit, # 使用8位量化来减少内存占用和加速推理
        # torch_dtype=load_type, # 选择数据精度
        # low_cpu_mem_usage=True, # 尝试在CPU上使用较少的内存。这可以有助于在资源受限的情况下进行推理。
        device_map='auto', # 它设置为 'auto'，表示让框架自动选择设备。
    )
    # 词汇表大小和分词器大小对应
